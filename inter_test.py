...
# Dictionnaire d'utilisateurs et mots de passe (pour l'authentification)
users = {
    'user1': 'password1',
    'user2': 'password2'
}
...
if 'username' not in st.session_state:
    ...
    if st.button("Se connecter"):
        if check_credentials(username, password):
            ...
            st.experimental_rerun()  # Forcer le rechargement de l'interface pour entrer dans l'application
        else:
            st.error("Nom d'utilisateur ou mot de passe incorrect.")
else:
    # Interface de l'application si l'utilisateur est authentifié
        ...
        with st.spinner('Chargement des prévisions...'):
            ...
            if model is None:
                ...
                if forecast_type == 'Mensuelles':
                    ...
                    future = pd.date_range(start=f'{year}-01-01', end=f'{year}-12-31', freq='M').to_frame(index=False,
                                                                                                          name='ds')
                    ...
                    predictions['Quantité prévue'] = predictions['Quantité prévue'].apply(
                    ...
                    for _, row in predictions.iterrows():
                        quantity_style = 'font-weight: bold; color: green;' if row['Quantité prévue'].replace(' ',
                                                                                                              '').isdigit() and int(
                            row['Quantité prévue'].replace(' ', '')) > 50000 else ''
